...
public class TattooStats
{
	...
	public boolean setTattoo(int tattooLocation, int tattooId, int tattooColour) {
		...
		TattooTracker.sendTattooUpdate(this.userId, tattooLocation, tattooId, tattooColour);
	...
	public boolean removeTattoo(int tattooLocation) {
		...
		TattooTracker.sendTattooUpdate(this.userId, tattooLocation, -1, 0);
	...
	public boolean drainCharge(int drain) {
		if(drain > tattooCharge) return false;
			...
			tattooCharge -= drain;
			TattooTracker.sendTattooChargeChange(this.userId, tattooCharge);
		}
		return true;
	}
	...
	public void loadFromNBT(EntityPlayer ePlayer) {
		...
		int[] storedColours = wmTags.getIntArray(PlayerNBTNames.TATTOO_COLOUR_LIST_NAME);
		for(int i=0; i < storedColours.length; i++) {
			tattooColours[i] = storedColours[i];
		}
	}
	
	private void saveTattooLocations(NBTTagCompound wmTags) {
		for(int i=0; i<TattooHandler.SlotIDToLocation.size(); i++) {
			String tattooName = "";
			if(tattooValues[i] != -1) {
	...
	public PacketTattooData assemblePacket() {
		return new PacketTattooData(this.userId, this.tattooCharge, this.tattooValues, this.tattooColours);
	}
